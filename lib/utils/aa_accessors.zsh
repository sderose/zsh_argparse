...
[ $ZERG_SETUP ] || source zerg_setup.zsh
...
EOF
    ...
    req_argc 2 2 $# || return $ZERR_ARGC
    is_of_zsh_type assoc "$1" assoc "$2" || return $ZERR_ZSH_TYPE
...
Note: There appears to be no way to set an item in a zsh assoc that
    is referenced indirectly, without using eval.
Note: Do not call this via backquites (`aa_set_default...`) or
    via $() (`$(aa_set_default...}`). Both of those run the function in a
    subshell, making it impossible to change the original shell's assoc.
EOF
            return ;;
        -q) quiet="-q" ;;
        --) shift ; break ;;
    ...
    if ! [ $? ]; then
        warn 0 "Eval failed: ${assoc_name}[\$key]=$qvalue"
        return $ZERR_EVAL_FAIL
    fi
}
...
EOF
    ...
    req_argc 3 3 $# || return $ZERR_ARGC
    is_of_zsh_type $quiet assoc "$1" || return $ZERR_ZSH_TYPE
