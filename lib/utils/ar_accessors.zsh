...
ar_clear() {
    ...
    is_of_zsh_type array "$1" || return ZERR_ZSH_TYPE
    typeset -a $1
    : ${(P)1::=()}
}
...
EOF
    ...
    req_argc 2 2 $# || return ZERR_ARGC
    is_of_zsh_type array "$1" || return ZERR_ZSH_TYPE
    aa_has zerg_types "$2" || return ZERR_ENUM
    local -i nitems=$(P)#1}
...
ar_get() {
    req_argc 2 2 $# || return ZERR_ARGC
    is_of_zsh_type array "$1" || return ZERR_ZSH_TYPE
    is_of_zerg_type integer "$2" || return ZERR_ZERG_TVALUE
    # slicing
    return ZERR_NOT_YET
}
...
ar_append() {
    req_argc 2 2 $# || return ZERR_ARGC
    is_of_zerg_type array "$1" || return ZERR_ZERG_TVALUE
    return ZERR_NOT_YET
}
