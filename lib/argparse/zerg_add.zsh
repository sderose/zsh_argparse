...
EOF
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case "$1:l" in
            --type|-t)
                shift
                local tname="$1" topts=""
                if [[ $tname =~ ([a-zA-Z0-9_]+ -) ]]; then  # options...
                    topts=${tname#* }; tname=${tname%% *}
                fi
                name=`zerg_opt_to_var -- "$1"`
                #warn 0 "Type '$name' for def $def_name."
                aa_has zerg_types "$tname" || return $ZERR_ENUM
                aa_set $def_name type "$tname"
                [ $topts ] && aa_set $def_name type_options "$topts" ;;
            ...
            --choices|-c)
                shift
                is_of_zerg_type idents "$1" || return $?
                aa_set $def_name choices "$1"
                aa_set_default $def_name action "store" ;;
            --const|-k)
            ...
            --dest|-v)
                ...
                is_of_zerg_type ident "$1" || return $?
            ...
            --format)
                shift
                is_of_zerg_type format "$1" || return $?
            ...
            --nargs|-n)
                shift
                [[ $1 == remainder ]] || is_of_zerg_type int "$1" || return $?
                aa_set $def_name nargs "$1" ;;
            --pattern|-x)
                shift
                is_of_zerg_type regex "$1" || return $?
    ...
    if [ -n "$type" ]; then
        if [ -n "$const" ]; then
            is_of_zerg_type "$type" "$const" || return $?
        fi
        if [ -n "$default" ]; then
            is_of_zerg_type "$type" "$default" || return $?
        fi
    fi
    ...
    #warn "====parser and argdef before zerg_add returns: "
...
EOF
    ...
    is_of_zerg_class ZERG_PARSER "$1" || return $?
    is_of_zerg_class ZERG_PARSER "$2" || return $?
...
EOF
    ...
    while [ -n "$1" ]; do
        is_of_zerg_class ZERG_ARG_DEF "$1" || return $?
